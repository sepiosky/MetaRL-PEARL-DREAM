...
class EncoderDecoder(Embedder, relabel.RewardLabeler):
    ...
    def _compute_contexts(self, trajectories):
        ...
        sequence_lengths = torch.tensor(
                [len(traj) for traj in trajectories], device='cpu').long()
    ...
    def _compute_losses(
        ...
        """
        decoder_context_loss = ((all_decoder_contexts - id_contexts.unsqueeze(dim=1).expand(*all_decoder_contexts.size()).detach())**2).sum(dim=-1) # .detach() after expand
        # ********************************************************
        # ******************* YOUR CODE HERE *********************
        # ********************************************************
    ...
    def label_rewards(self, trajectories):
        ...
        distances = ((all_decoder_contexts - id_contexts.unsqueeze(dim=1).expand(*all_decoder_contexts.size()).detach())**2).sum(dim=-1)
        rewards = distances[:,:distances.size(1)-1] - distances[:,1:] # telescoping series intrinsic reward
        # ********************************************************
        # ******************* YOUR CODE HERE *********************
        # ********************************************************
        ...
        # ******************* YOUR CODE HERE *********************
        # ********************************************************
        return ((rewards - self._penalty) * mask[:, 1:]), distances
